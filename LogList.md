...
- [x] ### Category;🌹🌹228(1h)—> 310(2.5h)==>311(0.7h)
...
3. 覆盖原先类中的方法。覆盖时并不是替换原来类中同名方法的实现，而是Category声明的方法在方法列表的前面，顺着列表调用的就是Category中的实现。 ps： 如果要调用被Category覆盖的方法，则需在方法列表中查找最后一个对应的方法名称，调用相应的实现即可。

**添加成员变量**: 

其实这里的类别特指OC中的Category，Category中无法声明属性，通过runtime中的objc_getAssociatedObject/objc_setAssociatedObject来访问和生成关联对象，模拟添加属性。
